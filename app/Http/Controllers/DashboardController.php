...
class DashboardController extends Controller
{
    public function index(CashierShiftService $cashierShiftService)
    {
        ...
        // Shared queries (used by all roles)
        ...
        // Super-admin only queries
        if ($user->isSuperAdmin()) {
            $totalRevenue = Transaction::sum('grand_total');
            $totalProfit = Profit::sum('total');
            $averageOrder = Transaction::avg('grand_total') ?? 0;

            $yesterdaySales = Transaction::whereDate('created_at', Carbon::yesterday())->sum('grand_total');
            $yesterdayProfit = Profit::whereDate('created_at', Carbon::yesterday())->sum('total');
        ...
        if ($user->isSuperAdmin()) {
            $pageData['totalRevenue'] = (int) $totalRevenue;
            $pageData['totalProfit'] = (int) $totalProfit;
            $pageData['averageOrder'] = (int) round($averageOrder);
            $pageData['yesterdaySales'] = (int) $yesterdaySales;
            $pageData['yesterdayProfit'] = (int) $yesterdayProfit;
            $pageData['yesterdayTransactions'] = (int) $yesterdayTransactions;
            $pageData['topLocations'] = $topLocations;
        }

        return Inertia::render('Dashboard/Index', $pageData);
    }
}
