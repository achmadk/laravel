    ...
    <>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="mb-2 flex items-center gap-2">
              <h1 className="font-bold text-2xl text-fg">{segment.name}</h1>
              <span className="inline-flex rounded-full bg-muted px-3 py-1 font-semibold text-muted-fg text-xs">
                {segment.type}
              </span>
            </div>
            <p className="text-muted-fg text-sm">{segment.description || "Segment CRM customer"}</p>
          </div>
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl bg-bg px-4 py-3 shadow-sm ring-1 ring-border">
              ...
              <p className="mt-1 font-bold text-fg text-xl">{segment.stats.total_members}</p>
            </div>
            <div className="rounded-2xl bg-bg px-4 py-3 shadow-sm ring-1 ring-border">
              <p className="text-xs uppercase tracking-widetext-muted-fg">Manual</p>
              <p className="mt-1 font-bold text-fg text-xl">{segment.stats.manual_members}</p>
            </div>
            <div className="rounded-2xl bg-bg px-4 py-3 shadow-sm ring-1 ring-border">
              <p className="text-xs uppercase tracking-widetext-muted-fg">Auto</p>
              <p className="mt-1 font-bold text-fg text-xl">{segment.stats.auto_members}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.7fr_1fr]">
        <div className="rounded-2xl border border-border bg-bg p-5">
          <h2 className="mb-4 font-semibold text-fg text-lg">Anggota Segment</h2>
            ...
            <tbody>
              {segment.memberships.length > 0 ? (
                segment.memberships.map((membership) => (
                  <tr key={membership.id} className="border-border border-b">
                    <td className="px-4 py-3">
                      ...
                      <p className="text-muted-fg text-xs">{membership.customer?.no_telp || "-"}</p>
                    </td>
                ...
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center">
                    ...
                    <p className="text-muted-fg text-sm">Belum ada anggota segment.</p>
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="space-y-6">
          {isManual && (
            <div className="rounded-2xl border border-border bg-bg p-5">
              <h2 className="mb-4 font-semibold text-fg text-lg">Tambah Anggota Manual</h2>
