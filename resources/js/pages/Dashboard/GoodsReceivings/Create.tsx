        ...
        <div className="space-y-6">
          <Card>
            <CardContent className="p-5">
              <h2 className="mb-4 font-semibold text-fg text-lg">Pilih Purchase Order</h2>
              <select
                value={selectedPoId}
              ...
              {errors.purchase_order_id && (
                <p className="mt-1 text-danger text-xs">{errors.purchase_order_id}</p>
              )}
            </CardContent>
          </Card>
          ...
          {selectedOrder && data.items.length > 0 && (
            <Card>
              <CardContent className="p-5">
                <h2 className="mb-4 font-semibold text-fg text-lg">Item Diterima</h2>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {data.items.map((item, index) => (
                        <tr key={item.purchase_order_item_id} className="border-border border-b">
                          ...
                          <td className="px-3 py-3 text-right">
                            <input
                              ...
                              className="h-10 w-24 rounded-lg border border-input bg-muted px-3 text-right text-fg text-sm outline-none transition focus:border-ring focus:ring-2 focus:ring-ring"
                            />
                          </td>
                          <td className="px-3 py-3 text-right">
                            <input
                              ...
                              placeholder="-"
                              className="h-10 w-32 rounded-lg border border-input bg-muted px-3 text-fg text-sm outline-none transition focus:border-ring focus:ring-2 focus:ring-ring"
                            />
                          </td>
                        </tr>
                      ))}
          ...
          {selectedOrder && data.items.length > 0 && (
            <Card>
              <CardContent className="p-5">
                <h2 className="mb-4 font-semibold text-fg text-lg">Catatan Penerimaan</h2>
                <textarea
                  value={data.notes}
          ...
          <div className="flex justify-end gap-3">
            ...
            {selectedOrder && data.items.length > 0 && (
              <button
                type="submit"
                disabled={processing}
                className="inline-flex items-center gap-2 rounded-xl bg-success px-4 py-2 font-semibold text-sm text-white shadow-lg shadow-success/30 hover:bg-success/90 disabled:cursor-not-allowed disabled:opacity-60"
              >
