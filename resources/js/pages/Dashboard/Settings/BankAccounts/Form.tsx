      ...
      <div className="max-w-3xl space-y-6">
        ...
        >
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              <label className="mb-2 block text-sm font-medium text-fg">Nama Bank</label>
              <input
              ...
              />
              {errors.bank_name && (
                <p className="mt-1 text-xs text-danger-500">{errors.bank_name}</p>
              )}
            </div>
            <div>
              <label className="mb-2 block text-sm font-medium text-fg">Nomor Rekening</label>
              <input
              ...
              {errors.account_number && (
                <p className="mt-1 text-xs text-danger-500">{errors.account_number}</p>
              )}
            </div>
          </div>
          <div>
            <label className="mb-2 block text-sm font-medium text-fg">Atas Nama</label>
            <input
            ...
            />
            {errors.account_name && (
              <p className="mt-1 text-xs text-danger-500">{errors.account_name}</p>
            )}
          </div>

          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              <label className="mb-2 block text-sm font-medium text-fg">Logo Bank (opsional)</label>
              <input
                type="file"
                accept="image/*"
                onChange={(e) => setData("logo", e.target.files?.[0] || null)}
                disabled={!canUpdatePaymentSettings}
                className="h-11 w-full rounded-xl border border-input bg-bg px-3 text-sm text-fg file:mr-3 file:rounded-lg file:border-0 file:bg-primary file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-primary-fg hover:file:bg-primary/90"
              />
              {errors.logo && <p className="mt-1 text-xs text-danger-500">{errors.logo}</p>}
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              href={settings.bankAccounts.index.url()}
              className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-2 text-sm text-muted-fg transition-colors hover:bg-muted"
            >
              Batal
            </Link>
          </div>
