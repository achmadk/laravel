...
export default function ThermalReceipt80mm({
  transaction,
  storeName = "TOKO ANDA",
  storeAddress = "",
  storePhone = "",
  ...
  return (
    <div
      className="font-mono text-xs leading-tight text-black"
      style={{ width: "80mm", padding: "4mm" }}
    >
      <style>
        {`
          @media print {
            .thermal-receipt { width: 80mm !important; padding: 0; margin: 0; }
          }
      ...
      <div className="mb-2 text-center">
        ...
        {storeAddress && <p className="text-[11px]">{storeAddress}</p>}
        {storePhone && <p className="text-[11px]">Telp: {storePhone}</p>}
        {storeEmail && <p className="text-[11px]">Email: {storeEmail}</p>}
        {storeWebsite && <p className="text-[11px]">{storeWebsite}</p>}
      </div>
